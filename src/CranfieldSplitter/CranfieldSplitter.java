...
public class CranfieldSplitter {
	...
	public static String analyze(String text) throws IOException {
		...
		synchronized(stop) {
			tokenStream = stop.tokenStream("contents", new StringReader(text));
		}
		termAttribute = tokenStream.getAttribute((Class<CharTermAttribute>) CharTermAttribute.class);
		...
		synchronized(snowball) {
			tokenStream = snowball.tokenStream("contents", new StringReader(tempString));
		}
		termAttribute = tokenStream.getAttribute((Class<CharTermAttribute>) CharTermAttribute.class);
		String returnString = "";
